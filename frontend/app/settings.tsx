...
export default function Settings() {
  ...
  const save = async () => {
    ...
    } catch { Alert.alert("Could not save"); } finally { setLoading(false); }
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
          <View style={{ alignItems: "center", padding: spacing.xl, marginBottom: spacing.lg }}>
            ...
            <Text style={[type.h3, { marginTop: spacing.md, color: colors.ink }]}>{name}</Text>
          ...
          <Text style={[type.overline, { marginBottom: spacing.sm, color: colors.inkMuted }]}>Profile</Text>
          ...
          <Text style={[type.overline, { marginBottom: spacing.sm, marginTop: spacing.md, color: colors.inkMuted }]}>Billing</Text>
